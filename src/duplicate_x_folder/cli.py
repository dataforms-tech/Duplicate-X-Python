...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="duplicate-x", description="Find and manage duplicate files by content."
    )
    p.add_argument(
        "--log-level",
        default="INFO",
    ...
    scan.add_argument(
        "--json", type=str, default=None, help="Write scan result to JSON file."
    )

    delete = sub.add_parser(
        "delete", help="Delete/move duplicate files (keeps one per group by default)."
    )
    delete.add_argument(
        "root", type=str, nargs="?", help="Root folder (used if rescanning)."
    )
    delete.add_argument(
        "--input",
        type=str,
        default=None,
        help="Use a previous scan JSON instead of rescanning.",
    )
    delete.add_argument(
        "--keep",
        default="first",
        choices=["first", "newest", "oldest", "shortest-path"],
        help="Which file to keep per group.",
    )
    delete.add_argument(
        "--interactive-keep",
        action="store_true",
        help="Choose the file to keep per group interactively.",
    )
    delete.add_argument(
        "--select",
        action="append",
        default=[],
        help="Only act on selected group:index (repeatable).",
    )
    delete.add_argument(
        "--mode",
        default="quarantine",
        choices=["delete", "trash", "quarantine"],
        help="What to do with duplicates.",
    )
    delete.add_argument(
        "--quarantine-dir",
        default=None,
        help="Quarantine folder (default: <root>/Duplicates_Quarantine).",
    )
    delete.add_argument(
        "--yes", action="store_true", help="Assume yes for confirmations."
    )
    delete.add_argument(
    ...
    merge = sub.add_parser(
        "merge", help="Move duplicates into a single folder under the root."
    )
    merge.add_argument(
        "root", type=str, nargs="?", help="Root folder (used if rescanning)."
    )
    ...
    merge.add_argument(
        "--merge-dir",
        default=None,
        help="Destination folder (default: <root>/Duplicates_Merged).",
    )
    merge.add_argument(
    ...
    gui.add_argument(
        "--log-level", default=None, help="Override global log level for GUI."
    )
    return p
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.cmd == "delete":
        ...
        if args.mode == "quarantine":
            quarantine = (
                ...
                else Path(result.root) / "Duplicates_Quarantine"
            )
        errs = delete_files(
            plan.to_act,
            mode=args.mode,
    ...
    if args.cmd == "merge":
        ...
        merge_dir = (
            Path(args.merge_dir)
            if args.merge_dir
            else Path(result.root) / "Duplicates_Merged"
        )
